...
for s = 1, screen.count() do
	...
	if taglist_numbers == 'random' then
		...
		tags[s] = awful.tag(taglist_numbers_sets[taglist_numbers], s, layouts[2])
	end
...
volbar.widget:buttons(awful.util.table.join(
   awful.button({ }, 1, function () exec("alsamixergui") end),
   awful.button({ }, 4, function () exec("amixer set Master playback 1+", false) vicious.force({volbar, volwidget}) end),
   awful.button({ }, 5, function () exec("amixer set Master playback 1-", false) vicious.force({volbar, volwidget}) end)
